    ...
    <div class="task-list-header">
      ...
      >
        <el-checkbox
          ...
          :label="type.value"
          :key="type.value"
          >{{ $t(type.name) }}</el-checkbox
        >
      </el-checkbox-group>
      <el-checkbox-group
        style="margin-left: 50px"
        v-model="checkedStatuss"
      ...
      <el-input
        ...
        v-model="keyword"
        style="margin-left: 12px; margin-right: 12px; width: 400px"
    ...
    <el-table border :data="tableData" style="width: 100%">
      ...
      <el-table-column :label="$t('操作')">
        <template v-slot="scope">
          <el-button
            size="mini"
            :disabled="scope.row.type != 'text' ? false : true"
            @click="handleBiaoZhu(scope.row)"
            >{{ $t('去标注') }}</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleBiaoZhu(task) {
      this.$router.push({
        path:
          '/project/task/resourceAnnotation?name=' +
