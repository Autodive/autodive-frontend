<template>
  <div id="resourceAnnotation" class="resource-annotation" style="height: 100%">
    <div class="content">
      <div :style="{ display: showMarkList ? 'none' : 'flex' }" class="pop-btn" @click="setMarkListDisplay(true)">
        <img src="@/assets/jtright.png" />
        <p class="rotate_text">{{ $t('查看标签') }}</p>
      </div>
      <div class="resource-annotation-left" :style="{ display: showMarkList ? 'block' : 'none' }">
        <mark-list ref="markListViewer" @clickDictBiaoZhu="clickDictBiaoZhu" @clickRegBiaoZhu="clickRegBiaoZhu" @deleteBiaozhu="delBiaoZhuFromMark" @deleteRelation="delRelationFromMark" @setAnnotationDisplay="setAnnotationDisplay" @setResourceZoneAnnotationActive="setResourceZoneAnnotationActive" @didGetNewBentiMarkListData="didGetNewBentiMarkListData" @didGetNewRelationMarkListData="didGetNewRelationMarkListData" />
      </div>

      <div v-if="showMarkList" class="pop-btn" @click="setMarkListDisplay(false)">
        <img src="@/assets/jtleft.png" />
        <p class="rotate_text">{{ $t('收起标签') }}</p>
      </div>

      <div class="resource-annotation-preview" :style="{
          width: showMarkList ? 'calc(100% - 423px)' : 'calc(100% - 35px)',
        }">
        <pdf-zone v-if="resource.type == 'paper'" ref="pdfzone" :resource="resource" :definRelationData="definRelationData" :bentiMarkList="bentiMarkListData" :relationMarkList="relationMarkListData" @finishedLoad="finishedLoad" @finishedSearch="finishedSearch" @getbz="getAnnotationFromPdf" @addRelationCaptionOfAnnotation="addRelationCaptionOfAnnotation" @getdelbq="getDelAnnotationFromOperationZone" @getgx="getRelationFromOperationZone" @editorAnnotation="editorAnnotation" @showbq="showbq" @getok="completeAction" @getzc="stagData" @setResourceMarkListAnnotationActive="
            setResourceMarkListAnnotationActive
          " />

        <image-zone v-if="resource.type == 'image'" ref="imagezone" :resource="resource" :definRelationData="definRelationData" :bentiMarkList="bentiMarkListData" :relationMarkList="relationMarkListData" @finishedLoad="finishedLoad" @getbz="getAnnotationFromPdf" @addRelationCaptionOfAnnotation="addRelationCaptionOfAnnotation" @getdelbq="getDelAnnotationFromOperationZone" @getgx="getRelationFromOperationZone" @editorAnnotation="editorAnnotation" @showbq="showbq" @getok="completeAction" @getzc="stagData" @setResourceMarkListAnnotationActive="
            setResourceMarkListAnnotationActive
          " />

        <txt-zone v-if="resource.type == 'txt'" ref="txtzone" :resource="resource" :definRelationData="definRelationData" :bentiMarkList="bentiMarkListData" :relationMarkList="relationMarkListData" @getbz="getAnnotationFromPdf" @addRelationCaptionOfAnnotation="addRelationCaptionOfAnnotation" @getdelbq="getDelAnnotationFromOperationZone" @getgx="getRelationFromOperationZone" @editorAnnotation="editorAnnotation" @showbq="showbq" @getok="completeAction" @getzc="stagData" @setResourceMarkListAnnotationActive="
            setResourceMarkListAnnotationActive
          " />
      </div>
    </div>

    <el-dialog class="annotation-editor" :title="annotationEditor.heard" v-model="annotationEditor.isShow" style="width: 40%;min-width:500px;">
      <template v-slot:title>
        <div class="dialog-title">
          <span style="font-weight: bold"><i :class="annotationEditor.typeIconClass" style="padding-right: 10px"></i>{{ annotationEditor.heard }}</span>
        </div>
      </template>
      <el-form>
        <el-form-item :label="$t('标题') + ' :'">
          <el-input :placeholder="$t('请输入标题')" v-model="annotationEditor.title"></el-input>
        </el-form-item>

        <el-form-item v-if="annotationEditor.annotation.annotationType == 'textBlock'" :label="$t('内容') + ' :'">
          <el-input :placeholder="$t('请输入内容')" :disabled="true" type="textarea" :autosize="{ minRows: 1, maxRows: 4 }" v-model="annotationEditor.annotation.content"></el-input>
        </el-form-item>

        <el-form-item :label="$t('实体标签') + ' :'">
          <el-input :placeholder="$t('请输入内容')" :disabled="true" v-model="annotationEditor.definName"></el-input>
        </el-form-item>

        <el-form-item v-if="annotationEditor.relatedUrl.length > 0" :label="
            $t(
          ...
          ">
          <img id="annotationEditorImg" :src="annotationEditor.relatedUrl + '?otpToken=' + otpToken" />
        </el-form-item>
        ...
        <el-form-item v-if="
          ...
          ">
          <el-input :placeholder="$t('请输入内容')" :disabled="true" type="textarea" :autosize="{ minRows: 1, maxRows: 4 }" v-model="
              annotationEditor.annotation.relatedCaptionPresentation.content
            "></el-input>
        </el-form-item>

        <el-form-item :label="$t('图片') + ' :'" v-if="annotationEditor.annotation.annotationType != 'textBlock'">
          <div class="editor-image">
            <el-image :src="annotationEditor.url" :preview-src-list="annotationEditor.previewList">
            </el-image>
          </div>
          <div class="images" v-if="
            ...
            ">
            <div style="display: flex" v-for="(url, index) in annotationEditor.images" :key="url">
              <img :src="`${baseURL}${url}?otpToken=${otpToken}`" />
              <i @click="deleteEditorImage(index)" style="margin-left: -10px" class="el-icon-close"></i>
            </div>
          </div>
        </el-form-item>

        <el-form-item class="annotation-editor-table-content" v-if="annotationEditor.annotation.annotationType == 'table'" :label="$t('表格内容') + ' :'">
          ...
          <template>
            <vue-excel-editor @cell-click="cellFocus" ref="table" no-footer v-model="annotationEditor.tabledata.data">
              <vue-excel-column v-for="(item, index) in annotationEditor.tabledata.cols" :key="index" :field="cn[index]" :label="cn[index]" type="string" width="80px" />
            </vue-excel-editor>
          </template>
        </el-form-item>

        <el-form-item :label="$t('元数据') + ' :'">
          <el-row style="margin: 10px 0px 0px 0px" :gutter="20" class="meta" v-for="(item, index) in annotationEditor.metadata" :key="index">
            <el-col :span="10">
            ...
            <el-col :span="4">
              <el-button @click="deleteMeta(index)" icon="el-icon-minus"></el-button>
            </el-col>
          </el-row>
          <el-button style="width: 100%; margin: 10px 0px 0px 0px" @click="addMeta" icon="el-icon-plus"></el-button>
        </el-form-item>
      </el-form>
...
import PdfZone from './components/pdfZone.vue'
import imageZone from './components/imageZone.vue'
import txtZone from './components/txtZone.vue'

import { baseURL } from '@/config'
import { getMattingBlob, uploadMattingBlob } from './components/matting.js'
import moshuViewer from './components/moshuViewer.vue'
...
export default {
  ...
  methods: {
    ...
    async requestResourceDetail() {
      ...
      this.loadingInstance = ElLoading.service({ target: target })
    ...
    async requestDefinRelation() {
      ...
      console.log(re, 're')
      this.definRelationData = re.data
    },
